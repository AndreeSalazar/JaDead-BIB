...
// C Native Compiler — Orchestrator
// ============================================================
// Connects: C Frontend → Optimizer → ISA Compiler → Backend
// ============================================================

use crate::backend::cpu::{elf, pe};
...
impl Builder {
    ...
    fn compile_program(
        ...
        let mut ub_detector = if options.warn_ub {
            UbDetector::new().with_strict()
        } else {
            UbDetector::new()
        };
        // TODO: UbDetector now works with JaCompilationUnit, not Program.
        // Skipping UB analysis for C builds until a C-specific detector is added.
        let _ub_reports: Vec<UBReport> = Vec::new();
    ...
    pub fn check_file(
        ...
        // TODO: UbDetector now works with JaCompilationUnit, not Program.
        // Skipping UB analysis for C builds until a C-specific detector is added.
        let detector = if warn_ub {
            UbDetector::new().with_strict()
        } else {
            UbDetector::new()
        };
        let reports: Vec<UBReport> = Vec::new();
